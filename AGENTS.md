...
- `/forge` Wave 5 已完成：T1.3.1、T3.2.1、T1.3.2、T1.3.3、INT-S3 已提交，code-reviewer 已落盘，结论 Partial Pass，最高残留 Medium（E2E 实机、tested 回填），v1 MVP 功能层面可关门。
- 后续优化项：用 `agent-browser` 为 31 个收录网站采集真实/美观示意截图，替换占位图；逐条复核网站并回填 `tested` 字段；执行 E2E 浏览器实机验证。
